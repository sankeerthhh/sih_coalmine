...
export const StatusBadge: React.FC<StatusBadgeProps> = ({ status, size = 'md' }) => {
  ...
  if (normalized === 'NORMAL' || normalized === 'ONLINE' || normalized === 'RESOLVED' || normalized === 'HEALTHY' || normalized === 'INTACT') {
    colorClasses = 'bg-emerald-50 text-emerald-800 border-emerald-300';
    dotColor = 'bg-emerald-600';
  } else if (normalized === 'WARNING' || normalized === 'DEGRADED' || normalized === 'ELEVATED') {
  ...
  } else if (normalized === 'CRITICAL' || normalized === 'DANGER' || normalized === 'ERROR' || normalized === 'SEVERED' || normalized === 'ACTIVE') {
